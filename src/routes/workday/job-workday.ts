...
export const jobWorkdayHandler: ExpressHandler = async (
  req: Request,
  res: Response,
  next: NextFunction
): Promise<void> => {
  ...
  try {
    ...
    const jobInfo = await page.evaluate(() => {
      ...
      // Função para extrair texto de elementos com data-automation-id
      ...
      // Extrair título
      const title = getTextContent('h2[data-automation-id="jobPostingHeader"]');
