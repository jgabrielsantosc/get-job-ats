...
export const scraperJobInhire = async (url: string): Promise<string[]> => {
  try {
    if (url.endsWith('/vagas') || url.endsWith('/vagas/')) {
      url = url.replace(/\/vagas\/?$/, '');
    }
    
    const tenant = extractTenantName(url);
    
    if (!tenant) {
      throw new Error('Tenant não encontrado na URL');
    }
    ...
    if (!response.data?.jobsPage) {
      console.warn(`Nenhuma vaga encontrada para o tenant ${tenant}`);
      return [];
    }

    const urls = response.data.jobsPage
      .filter(job => job.status === 'published')
      .map(job => createJobUrl(tenant, job.jobId, job.displayName));

    console.log(`Encontradas ${urls.length} vagas para ${tenant}`);
    return urls;
